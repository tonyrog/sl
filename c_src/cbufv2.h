...
static UNUSED size_t cbuf_r_avail(cbuf_t* cp) 
{
...
static UNUSED void cbuf_initv(cbuf_t* cp, ErlIOVec* vec)
{
    int i;
    cp->flags = 0;
...
static UNUSED void cbuf_forward(cbuf_t* cp, size_t len)
{
    while(cp->iv < cp->vlen) {
